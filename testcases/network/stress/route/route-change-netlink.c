...
void save_ip(void **data, const char *item)
{
	...
	tst_setup_addrinfo(item, p_opt, &hints, &n->ip);
	n->next = *list;
	*list = n;
}
...
static void rtnl_route(int iface, struct addrinfo *dst, struct addrinfo *gw, uint32_t prefix, int type)
{
	...
	if (gw) {
		if (is_ipv6) {
			gw_in6 = ((struct sockaddr_in6 *)dst->ai_addr)->sin6_addr;
			mnl_attr_put(nlh, RTA_GATEWAY, sizeof(struct in6_addr), &gw_in6);
		} else {
			gw_ip = ((struct sockaddr_in *)dst->ai_addr)->sin_addr.s_addr;
...
static void run(void)
{
	...
	for (i = 0; i < num_loops; i++) {
		...
		*/
		...
		tst_res(TINFO, "!!! gw: %s, rhost: %s, dst: %s, iface: %s",
					p_gw->ip_str, p_rhost->ip_str, p_dst->ip_str, p_iface->iface);

		rtnl_route(iface->index, p_dst->ip, p_gw->ip, prefix, RTM_NEWROUTE);
		send_udp(p_rhost->ip);
		rtnl_route(iface->index, p_dst->ip, p_gw->ip, prefix, RTM_DELROUTE);

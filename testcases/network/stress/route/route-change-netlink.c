...
static void run(void)
{
	...
	for (i = 0; i < num_loops; i++) {
		...
		rtnl_route(p_iface->index, p_dst->ip, gw ? p_gw->ip : NULL, prefix, RTM_NEWROUTE);
		send_udp(p_rhost->ip);
		rtnl_route(p_iface->index, p_dst->ip, gw ? p_gw->ip : NULL, prefix, RTM_DELROUTE);

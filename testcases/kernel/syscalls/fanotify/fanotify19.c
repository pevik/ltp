...
static void test_fanotify(unsigned int n)
{
	...
	if (fd_notify < 0) {
		...
		tst_brk(TBROK | TERRNO, "fanotify_init(FAN_CLASS_NOTIF, O_RDONLY) failed");
	}
	...
	if (fanotify_mark(fd_notify, FAN_MARK_ADD, FAN_ALL_EVENTS,
		...
		tst_brk(TBROK | TERRNO,
			"fanotify_mark(%d, FAN_MARK_ADD, %d, AT_FDCWD, %s) failed",
			fd_notify,
			FAN_ALL_EVENTS,
			TEST_FILE);
	}
	...
	while (FAN_EVENT_OK(event, len) && test_number < tc->event_count) {
		if (!(event->mask & tc->event_set[test_number])) {
			tst_res(TFAIL,
				"Received unexpected event mask: mask=%llx pid=%u fd=%d",

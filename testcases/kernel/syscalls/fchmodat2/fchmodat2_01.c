 ...
 * regular files, symbolic links and directories.
 *
 * AT_SYMLINK_NOFOLLOW is a special feature that is not enabled in VFS for
 * symbolic links, so we only verify that EOPNOTSUPP is correctly raised when
 * used on those particular files. Changed in 5d1f903f75a8 ("attr: block mode
 * changes of symlinks").
 */

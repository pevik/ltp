...
sub get_letters
{
	...
	#my (@names) = sort shift;
=cut
	print "in: "; # FIXME: debug
	print Dumper(@names); # FIXME: debug
=cut
	my $letter;
	my $prev_letter = "";
	my $content;
...
=cut
	print "pred: "; # FIXME: debug
	print Dumper(keys %{$json->{'tests'}}); # FIXME: debug
=cut
...
sub content_tags
{
	...
	for my $k (sort @$data) {
		$content .= "<span>";
		$content .= test_url($k, $json->{'tests'}->{$k});
		$content .= "</span> ";
	}
...
=cut
	...
	print Dumper($data);
	print "data0 ($$data[0]): ";
	print Dumper(%{$json->{'tests'}}{$$data[0]});
	print "data0 (2) ($$data[0]): ";
	print Dumper($json->{'tests'}->{$$data[0]});
	print "data1 ($$data[1]): ";
	print Dumper($json->{'tests'}->{$$data[1]});
=cut
...
for my $k (sort keys %$data) {
	my %c = (file => text2url($k), subtitle => tag2title($k), content => \&content_tags, data => $k);
	push @{$config}, \%c;
}

for my $c (@{$config}) {
	open(my $fh, '>', $c->{'file'}) or die("Can't open $c->{'file'} $!");
	print_html($fh, $json, $c->{'subtitle'}, $c->{'content'}->($json,
			defined($c->{'data'}) ? $data->{$c->{'data'}} : undef), $config);
}

...
test_exec test_timer tst_bool_expr tst_res_hexd tst_strstatus tst_fuzzy_sync01
tst_fuzzy_sync02}"
...
usage()
{
	cat << EOF
Usage: $0 [-b DIR ] [-c|-s]
-b DIR  build directory (useful for out-of-tree build)
...
run_tests()
{
	...
	for i in $vars; do
		...
		case $ret in
			...
			32) tconf="$tconf $i";;
			127) tst_brk TBROK "Error: file not found (wrong PATH?), exit code: $ret";;

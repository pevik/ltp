...
if test "x$enable_metadata" = xyes || test "x$enable_metadata_html" = xyes -o "x$enable_metadata_pdf" = xyes; then
	...
	if test "x$with_metadata_generator" = xdetect; then
		...
		if test "x$metadata_generator_html" = x -a "x$enable_metadata_html" != xyes ||
			...
			LTP_CHECK_METADATA_GENERATOR_ASCIIDOC
			if test "x$metadata_generator_html" = x -a "x$backup_html" != x -a "x$enable_metadata_html" != xyes ||
				test "x$metadata_generator_pdf" = x -a "x$backup_pdf" != x -a "x$enable_metadata_pdf" != xyes; then
				with_metadata_generator='asciidoctor'
			fi
		fi
